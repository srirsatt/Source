...
function writeAntigravityGeneralRule(workspacePath) {
...
function writeAntigravityMcpConfig(workspacePath, extensionPath) {
...
// CLAUDE.md config
function writeClaudeMD(workspacePath) {
    const sourceDir = path.join(workspacePath, '.source');
    const manifestPath = path.join(sourceDir, 'manifest.json');
    if (!fs.existsSync(manifestPath)) {
        return;
...
function writeClaudeCodeMcpConfig(workspacePath, extensionPath) {
...
function writeAgentRules(pages, workspacePath, sourceUrl, extensionPath) {
    // antigravity
